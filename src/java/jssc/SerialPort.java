/*
 * jSSC (Java Simple Serial Connector) - serial port communication library. © Alexey Sokolov (scream3r), 2010-2014.
 * 
 * This file is part of jSSC.
 * 
 * jSSC is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser General Public
 * License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later
 * version.
 * 
 * jSSC is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty
 * of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more details.
 * 
 * You should have received a copy of the GNU Lesser General Public License along with jSSC. If not, see
 * <http://www.gnu.org/licenses/>.
 * 
 * If you use jSSC in public project you can inform me about this by e-mail, of course if you want it.
 * 
 * e-mail: scream3r.org@gmail.com web-site: http://scream3r.org | http://code.google.com/p/java-simple-serial-connector/
 */
package jssc;
...
public class SerialPort {
	...
	public SerialPort(String portName, SerialInterfaceType serialInterfaceType) {
		this.portName = portName;
		this.serialInterface = SerialInterfaceFactory.getInterface(serialInterfaceType);
	}
	...
	public boolean openPort() throws SerialPortException {
		...
		if (portName != null) {
			...
			throw new SerialPortException(portName, "openPort()", SerialPortException.TYPE_NULL_NOT_PERMITTED);// since
																												// 2.1.0
																												// ->
																												// NULL
																												// port
	 ...
	 * <b>"PURGE_"</b>, for example <b>"PURGE_RXCLEAR"</b>. Sent parameter "flags" is additive value, so addition of
	 * flags is allowed. For example, if input or output buffer shall be purged, parameter
	 * <b>"PURGE_RXCLEAR | PURGE_TXCLEAR"</b>. <br>
	...
	/**
	 * Set events mask. Required flags shall be sent to the input. Variables with prefix <b>"MASK_"</b>, shall be used
	 * as flags, for example <b>"MASK_RXCHAR"</b>. Sent parameter "mask" is additive value, so addition of flags is
	 * allowed. For example if messages about data receipt and CTS and DSR status changing shall be received, it is
	 * required to set the mask - <b>"MASK_RXCHAR | MASK_CTS | MASK_DSR"</b>
	 * 
	 * @return If the operation is successfully completed, the method returns true, otherwise false
	 *
	 * @throws SerialPortException
	...
	private void waitBytesWithTimeout(String methodName, int byteCount, int timeout) throws SerialPortException,
	...
	/**
	 * Set flow control mode. For required mode use variables with prefix <b>"FLOWCONTROL_"</b>. Example of hardware
	 * flow control mode(RTS/CTS): setFlowControlMode(FLOWCONTROL_RTSCTS_IN | FLOWCONTROL_RTSCTS_OUT);
	 *
	 * @return If the operation is successfully completed, the method returns true, otherwise false
	 *
	 * @throws SerialPortException
	...
	/**
	 * Add event listener. Object of <b>"SerialPortEventListener"</b> type shall be sent to the method. This object
	 * shall be properly described, as it will be in charge for handling of occurred events. This method will
	 * independently set the mask in <b>"MASK_RXCHAR"</b> state if it was not set beforehand
	 *
	 * @throws SerialPortException
	 */
	...
	/**
	 * Add event listener. Object of <b>"SerialPortEventListener"</b> type shall be sent to the method. This object
	 * shall be properly described, as it will be in charge for handling of occurred events. Also events mask shall be
	 * sent to this method, to do it use variables with prefix <b>"MASK_"</b> for example <b>"MASK_RXCHAR"</b>
	 *
	...
	/**
	 * Internal method. Add event listener. Object of <b>"SerialPortEventListener"</b> type shall be sent to the method.
	 * This object shall be properly described, as it will be in charge for handling of occurred events. Also events
	 * mask shall be sent to this method, to do it use variables with prefix <b>"MASK_"</b> for example
	 * <b>"MASK_RXCHAR"</b>. If <b>overwriteMask == true</b> and mask has been already assigned it value will be
	 * rewrited by <b>mask</b> value, if <b>overwriteMask == false</b> and mask has been already assigned the new
	 * <b>mask</b> value will be ignored, if there is no assigned mask to this serial port the <b>mask</b> value will be
	 * used for setting it up in spite of <b>overwriteMask</b> value
	 *
	 * @see #setEventsMask(int) setEventsMask(int mask)
	 *
	 * @throws SerialPortException
	...
	/**
	 * Delete event listener. Mask is set to 0. So at the next addition of event handler you shall set required event
	 * mask again
	 *
	 * @return If the operation is successfully completed, the method returns true, otherwise false
	 * 
			...
			while (!threadTerminated) {
				...
				for (int i = 0; i < eventArray.length; i++) {
					if (eventArray[i][0] > 0 && !threadTerminated) {
						...
						// FIXME
						/*
						 * if(methodErrorOccurred != null){ try { methodErrorOccurred.invoke(eventListener, new
						 * Object[]{new SerialPortException("port", "method", "exception")}); } catch (Exception ex) {
					...
					if (eventType > 0 && !super.threadTerminated) {
						switch (eventType) {
							...
							case MASK_RXCHAR:
								...
								break;
							/*
							 * case MASK_RXFLAG: //Do nothing at this moment if(((mask & MASK_RXFLAG) == MASK_RXFLAG) &&
